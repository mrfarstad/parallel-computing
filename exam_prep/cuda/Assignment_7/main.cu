...
int main(int argc, char **argv) {
  ...
  cudaErrorCheck(cudaMalloc((void **)&final_data,
                            imageChannel->width * imageChannel->height));
  cudaErrorCheck(cudaMemcpy(final_data, imageChannel->rawdata,
                            imageChannel->width * imageChannel->height,
                            cudaMemcpyHostToDevice));
  cudaErrorCheck(cudaMalloc((void **)&working_data,
                            imageChannel->width * imageChannel->height));

  cudaErrorCheck(
      cudaMalloc((void **)&dev_filter, filter_dim * filter_dim * sizeof(int)));
  cudaErrorCheck(cudaMemcpy((void *)dev_filter, laplacian1Filter,
                            filter_dim * filter_dim * sizeof(int),
                            cudaMemcpyHostToDevice));
  ...
  cudaErrorCheck(cudaFree(dev_filter));
  cudaErrorCheck(cudaFree(final_data));
  cudaErrorCheck(cudaFree(working_data));
