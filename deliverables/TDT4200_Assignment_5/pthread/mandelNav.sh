...
TIME=$(/usr/bin/time -f "%E" $BIN -q -r $RES -x $X -y $Y -p $THREADS -s $S -i $I -c $cI -b $BLOCKDIM -d $SUBDIV -o $OUT $MARK $TRADITIONAL 2>&1 >/dev/null)
[ $? -ne 0 ] && {
...
function help() {
    ...
    printf "[W] subdivision up     [w] subdivision down\n"
    ...
    printf "Currently calling: %s\n" "$BIN -q -r $RES -x $X -y $Y -p $THREADS -s $S -i $I -c $cI -b $BLOCKDIM -d $SUBDIV -o $OUT $MARK $TRADITIONAL"
