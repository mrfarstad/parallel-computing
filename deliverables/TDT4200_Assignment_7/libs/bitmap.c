...
int reallocateBmpBuffer(bmpImage *image, unsigned int const width,
...
int reallocateBmpChannelBuffer(bmpImageChannel *image, unsigned int const width,
                               unsigned int const height) {
  freeBmpChannelData(image);
  if (height * width > 0) {
    image->rawdata =
        calloc(image->height * image->width, sizeof(unsigned char));
    if (image->rawdata == NULL) {
      return 1;
    }
...
bmpImageChannel *newBmpImageChannel(unsigned int const width,
                                    unsigned int const height) {
...
int loadBmpImage(bmpImage *image, char const *filename) {
  ...
  if (fread(header, sizeof(unsigned char), BMP_HEADER_SIZE, fImage) <
      BMP_HEADER_SIZE) {
    goto failed_read;
  }
  ...
  for (unsigned int y = 0; y < image->height; y++) {
    if (fread(data, sizeof(unsigned char), paddedLineSize, fImage) <
        paddedLineSize) {
      goto failed_read;
    }
...
int saveBmpImage(bmpImage *image, char const *filename) {
  ...
  unsigned char header[BMP_HEADER_SIZE] = {'B',
                                           'M',
                                           size & 255,
                                           (size >> 8) & 255,
                                           (size >> 16) & 255,
  ...
  if (fwrite(header, sizeof(unsigned char), BMP_HEADER_SIZE, fImage) <
      BMP_HEADER_SIZE) {
    ret = 1;
  } else {
    for (unsigned int i = 0; i < image->height; i++) {
      if (fwrite(image->data[i], sizeof(pixel), image->width, fImage) <
          image->width) {
        ret = 1;
        break;
      }
...
int extractImageChannel(bmpImageChannel *to, bmpImage *from,
...
int mapImageChannel(bmpImage *to, bmpImageChannel *from,
...
unsigned char extractRed(pixel from) { return from.r; }
unsigned char extractGreen(pixel from) { return from.g; }
unsigned char extractBlue(pixel from) { return from.b; }
